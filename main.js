...
function getCardHTML(product) {
  ...
  <div class="card-body">
    ...
    <a href="#" class="btn btn-primary" data-product='${JSON.stringify(product)}'>Дотати в кошик</a>
  </div>
</div>`
};
...
class ShoppingCart {
    ...
    saveCartToCookies() { let cartJSON = JSON.stringify(this.items);
document.cookie = `cart=${cartJSON}; max-age=${60 * 60 * 24 * 7}; path=/`;
    }

    loadCartFromCookies() {let cartCookie = getCookieValue('cart');
